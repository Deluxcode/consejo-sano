...
$(document).ready(function() {
  ...
  // Check offset point on load
  addHeader($(window).scrollTop());

  // Change offset point on scroll
  $(window).scroll(function() {
    addHeader($(this).scrollTop());
  });

  // Function to change bg for header
  ...
  $('.modal_slider').owlCarousel({
    ...
    smartSpeed: 1000,
    navText: [
      '<p class="slider-nav-btn prev-btn"><i class="fa fa-angle-left" aria-hidden="true"></i></p>',
      '<p class="slider-nav-btn next-btn"><i class="fa fa-angle-right" aria-hidden="true"></i></p>'
    ]
  });
  $(document).on('click', '.open_modal', function() {
      var position = $(this).attr('data-index');
      $('.modal_slider').trigger('to.owl.carousel', [position, 0]);
    });
  $(window).on("load",function(){
      $('.member_description').mCustomScrollbar({
          theme:'dark'
      });
  });
  ...
  // Custom select
  $('.custom_select_value').on('click', function(){
    $('.custom_select').removeClass('active');
    $(this).closest('.custom_select').addClass('active');
  });
  $('.custom_select li').on('click', function(){
    $(this).closest('.custom_select').find('.custom_select_value').text($(this).text());
    $(this).closest('.custom_select').find('.custom_select_input').text($(this).attr('data-value'));
  ...
  // Pain points clear
  $('.pain_point_text').focus(function(){
      $('.pain_points .radio_input').prop('checked', false);
  });
